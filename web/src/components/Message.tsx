...
// Thinking summary component (collapsible like ChatGPT)
interface ThinkingSummaryProps {
  summary: string;
  isCleanStyle: boolean;
}

function ThinkingSummary({ summary, isCleanStyle }: ThinkingSummaryProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  // Extract duration from summary (e.g., "8s 동안 생각함" -> "8s", "Thought for 5 seconds" -> "5 seconds")
  const duration = summary;

  return (
    <button
      onClick={() => setIsExpanded(!isExpanded)}
      className="flex items-center gap-2 mb-3 px-3 py-2 rounded-lg transition-colors cursor-pointer"
      style={{
